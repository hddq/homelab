...
echo -e "${GREEN}Download Migration Script ${VERSION}${NC}"

# Usage: ./download-from-pvc.sh <app-name> <namespace> <pvc-name> <local-path> <remote-path>
...
if [[ -z "$APP_NAME" || -z "$NAMESPACE" || -z "$PVC_NAME" || -z "$LOCAL_PATH" || -z "$REMOTE_PATH" ]]; then
    ...
    echo -e "${YELLOW}Example (Folder):${NC} $0 opodsync opodsync opodsync-pvc ./my-data-backup ."
    exit 1
fi
...
echo -e "${BLUE}📥 Downloading /mnt/data/$REMOTE_PATH to $LOCAL_PATH...${NC}"
if [[ ! -d "$LOCAL_PATH" ]]; then
    mkdir -p "$(dirname "$LOCAL_PATH")"
fi
kubectl cp "${NAMESPACE}/${TEMP_POD}:/mnt/data/$REMOTE_PATH" "$LOCAL_PATH"

echo -e "${GREEN}✅ Download complete. Verifying locally:${NC}"
ls -lhd "$LOCAL_PATH"
